...
from .pseudo_ground_truth import PseudoGroundTruthBuilder, PseudoGroundTruthCache
...
class Runner:
    def __init__(self, config_path: Path, use_pseudo_ground_truth: bool = False, save_raw: bool = True):
        ...
        if use_pseudo_ground_truth:
            ...
            self.pseudo_gt_cache = PseudoGroundTruthCache(ROOT / "results" / "pseudo_gt_cache")
        ...
        for target in tqdm(targets, desc="Targets"):
            ...
            if self.use_pseudo_ground_truth:
                ...
                if pseudo_gt and pseudo_gt.get('overall_confidence', 0) > 0.6:
                    ...
                    # Re-evaluate with pseudo ground truth
                    for method in methods:
                        if method in target_results:
                            for r in range(len(target_results[method])):
                                eval_res = self.evaluator.evaluate_single(
                                    enhanced_target, method, target_results[method][r]
                                )
                                ...
                                eval_res['consensus_metadata'] = {
                                    'scrapers_used': pseudo_gt['scrapers_used'],
                                    'coverage': pseudo_gt['stats']['coverage']
                                }
    ...
    def _build_pseudo_ground_truth_for_target(self, target: dict, target_results: dict):
        ...
        # Check cache first
        cached_pseudo_gt = self.pseudo_gt_cache.get(url)
        if cached_pseudo_gt:
            return cached_pseudo_gt
        ...
        if pseudo_gt.get('overall_confidence', 0) > 0.7:
            self.pseudo_gt_cache.put(url, pseudo_gt)
